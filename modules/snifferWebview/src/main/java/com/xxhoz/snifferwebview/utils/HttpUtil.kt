package com.xxhoz.secboxbrowser.utils
...
//                LogUtils.d("网络请求URL:" + url+ "  响应内容: "+ responseBody )
                Log.d("网络请求URL:" + url + "  响应内容: ", responseBody.toString())
                return gson.fromJson(responseBody, clazz)
            }
        } catch (e: Exception) {
//            LogUtils.e("网络请求错误URL:" + url)
            Log.d("网络请求错误URL:" + url, e.toString())
            e.printStackTrace()
            throw e
        }
    }
        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            if ("deflate".equals(originalResponse.header("Content-Encoding"), true)) {
                ...
                val inflaterInputStream =
                    InflaterInputStream(ByteArrayInputStream(originalBody?.bytes()), inflater)
                val uncompressedBody = inflaterInputStream.readBytes()
                    .toResponseBody(originalBody?.contentType())
